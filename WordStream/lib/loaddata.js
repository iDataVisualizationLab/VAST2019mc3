...
function loadVAST(draw, top){
    const formatTimeLegend = d3.time.format("%B %d, %-I:%M:%S %p");
    const fisrt5hrsRange = [1586201491000, 1586219491000];
    var topics = [];
    d3.json("data/test5hrs.json", function(error, rawData) {
        if (error) throw error;
        console.log(rawData);
        rawData.forEach((d,i) => {
            // if (i === 0){
            //     d.date = formatTimeLegend(new Date(fisrt5hrsRange[0]+i*60*60*1000))
            // }
            d.words.message = d.words.message.slice(0,70);
        });
        processSudden(rawData);
        draw(rawData);
    });
}