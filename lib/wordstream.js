    ...
    window.wordstream = function (svg, data, config) {
        d3.wordstreamLayout = function () {
            let data = [],
                ...
                smallStreamScale = d3.scaleLinear(),
            ...
            let maxSud, minSud, maxFreq, minFreq, totalFrequencies;
            wordstream.boxes = function () {
                ...
                // totalFrequencies = number of posts
                totalFrequencies = calculateTotalFrequenciesByPost(data);
                buildFontScale(data);
            ...
            function buildFontScale(data) {
                ...
                fontScale.domain([minFrequency, 286]).range([minFontSize, maxFontSize]);
            }

            function buildFrequencyScale(data) {
                let maxPostVertical = 0;
                let keys = d3.keys(totalFrequencies[0]);
                d3.map(totalFrequencies, function (d) {
                    let total = 0;
                    keys.forEach(key => {
                        // add vertically, per timestep
                        total += d[key];
                    });
                    if (total > maxPostVertical) maxPostVertical = total;
                });
                // max of vertical: streamFreq, not wordFreq
                frequencyScale.domain([0, maxPostVertical]).range([0, size[1]]);

                // old scale ----------------------------------
                let totalFrequenciesByWord = calculateTotalFrequenciesABox(data);
                let maxWord = 0;
                d3.map(totalFrequenciesByWord, function (d) {
                    let totalWord = 0;
                    keys.forEach(key => {
                        totalWord += d[key];
                    });
                    if (totalWord > maxWord) maxWord = totalWord;
                });
                smallStreamScale.domain([0, maxWord]).range([0, size[1]]);
            }
                ...
                for (let i = 0; i < data.length; i++) {
                    boxes.topics.forEach(topic => {
                        ...
                        while (++di < n) {
                            ...
                            d.y0 = -d.y1;
                            d.timeStep = i;
                            d.streamHeight = smallStreamScale(d.frequency);
                            x += w;
                        }
                    });
                }
            ...
            function calculateTotalFrequenciesByPost(data) {
                // this function can vary based on purpose
                // this version is used for VAST challenge 2019,
                let topics = d3.keys(data[0].words);
                let totalFrequencies = Array();
                data.forEach(d => {
                    let verticalFrame = {};
                    verticalFrame[topics[1]] = 0;
                    d.words["location"].forEach(word => {
                        verticalFrame[topics[1]] += word.frequency;
                    });
                    verticalFrame[topics[0]] = verticalFrame[topics[1]];
                    totalFrequencies.push(verticalFrame);
                });
        ...
        function getColor(d) {
            ...
            else if (d.topic === "location") return topicColor[1];
            else if (dataOption[0].id === otherPostID) return taxonomy.find(d => d.id === otherPostID).color;
            else {
                let finding = dataOption.find(e => {
                    return e.content.findIndex(e => {
                        return d.text.indexOf(e) >= 0
                    }) >= 0
                });
                return finding ? finding.color : topicColor[0]
            }
        }
        ...
        (function (data) {
            ...
            const font = "Arial";
            const specialPallete = d3.schemeCategory10.slice(0, 4).concat(color[0]);
            const arc = d3.arc()
                .outerRadius(5)
                .innerRadius(0);
            const pie = d3.pie()
                .sort(null)
                .value((d, i, a) => i === a.length - 1 ? a.length - 1 : 1);
            ...
            mainGroup = svg.append('g').attr("id", "main");
            ...
            wordstreamG = mainGroup.append('g').attr("id", "wordstreamG");
            ...
            mainGroup.selectAll('.textData')
                .on('mouseenter', function (d) {
                    ...
                    let prevOpacity = thisText.attr("fill-opacity")
                    ...
                    let info = tooltipInfo(d, wsRawData);
                    // append close button

                    if (topic === "location"){
                        prevMap =  +d3.select("#map" + removeChar(text)).style("fill-opacity")
                ...
                cornerButton.transition()
                        .duration(100)
                    .style("visibility", "visible");

                    createTableTooltip(wsTooltipDiv, info, text, prevColor, topic, d);
                    // format table position
                    let tableHeight = document.getElementById("tableTooltip")
                        .offsetHeight;

                    let tableWidth = document.getElementById("tableTooltip")
                        .offsetWidth;

                    wsTooltipDiv
                        .style("left", () => {
                            let mouseX = (d3.event.pageX);
                            let calculated = (mouseX < 2*wsContainerWidth(numHourAfter)/3)? (mouseX+50) : (mouseX - tableWidth - 50);
                            return Math.max(0, calculated) + "px";
                        })
                        .style("top", () => {
                            let calculated;
                            if (d.topic === "message"){
                                calculated = tableHeight > 500 ?
                                    (d3.event.pageY - 500 - 20) : (d3.event.pageY - tableHeight - 20)
                            }
                            else calculated = d3.event.pageY + 20;

                            return (Math.max(0, calculated)+400) + "px";
                        });
                    ...
                    cornerButton
                        .style("left", () => {
                            let mouseX = (d3.event.pageX);
                            let calculated = (mouseX < 2*wsContainerWidth(numHourAfter)/3)? (mouseX+50) : (mouseX - tableWidth - 50);
                            return (Math.max(0, calculated) + tableWidth - 18) + "px";
                        })
                        .style("top", () => {
                            let calculated;
                            if (d.topic === "message"){
                    ...
                    xButton
                        .style("left", () => {
                            let mouseX = (d3.event.pageX);
                            let calculated = (mouseX < 2*wsContainerWidth(numHourAfter)/3)? (mouseX+30) : (mouseX - 45);
                            return Math.max(0, calculated) + "px";
                        })
                        .style("top", () => {
                            let calculated = d.topic === "message"?
                                (d3.event.pageY - 10) :
                                (d3.event.pageY + 10);
                    ...
                    // get timestep
                    let timestep = nearestHour(info[0].time);
                    highlight(info, wsData, timestep);
            ...
            //Build the legends
            legendGroup
                .attr('transform', 'translate(' + margins.left + ',' + (height + margins.top + legendOffset) + ')');

            const legendContent = ["co-occurrence words", "location"];
            let legendNodes = legendGroup.selectAll('g')
                .data(legendContent)
                .enter().append('g')
                .attr('transform', function (d, i) {
            ...
            // legendNodes.select("#legendCircle0").remove();
