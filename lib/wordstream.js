(function (window) {
    window.wordstream = function (svg, data, config) {
        ...
        function getColor(d, option) {
            if (d.topic === "location") return topicColor[1];
            else {
                let c = eval(option + "Keyword")
                    .findIndex(type => {
                        return type.findIndex(word => {
                            return d.text.indexOf(word) >= 0
                        }) >= 0
                    });
                return c > -1 ? d3.schemeCategory10[c] : topicColor[0]
            }
        }
        (function (data) {
            ...
            textEnter
                ...
                .attr("fill", function (d, i) {
                    if (option === "resource"){
                        return getColor(d, option);
                    }
                    else {
                        return getColor(d, "event");
                    }
                })
            ...
            mainGroup.selectAll('.textData')
                ...
                wsTooltip.transition()
                    .duration(100)
                    .style("opacity", 1);

                wsTooltip.html(
                    '<text>' + info.map(d => d.message + "<br>") + "</text>")
            ...
            let pieG = legendGroup.append("g")
                .attr('transform', function (d, i) {
                return 'translate(' + 10 + ',' + (i * legendFont) + ')';
            });

            let newPie = pieG.selectAll(".arc")
                .data(pie(specialPallete))
                .enter().append("g")
                .attr("class", "arc");
            ...
            newPie.append("path")
                ...
                .style("stroke", "black")
                .attr("stroke-width", 0.1)
                .style("fill", function(d,i) {return specialPallete[i]; });
